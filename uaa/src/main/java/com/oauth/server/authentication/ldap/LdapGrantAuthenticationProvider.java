...
@Slf4j
public class LdapGrantAuthenticationProvider implements AuthenticationProvider {
    ...
    public LdapGrantAuthenticationProvider(OAuth2AuthorizationService authorizationService,
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        LdapGrantAuthenticationToken passwordGrantAuthenticationToken =
                (LdapGrantAuthenticationToken) authentication;
        ...
        // fixme 验证ldap密码有问题
//        BaseLdapPathContextSource baseLdapPathContextSource = new DefaultSpringSecurityContextSource("ldap://localhost:8389/dc=springframework,dc=org");
...
//        passwordComparisonAuthenticator.setUsePasswordAttrCompare(true);
...
//
//        LdapPasswordComparisonAuthenticationManagerFactory factory = new LdapPasswordComparisonAuthenticationManagerFactory(baseLdapPathContextSource, new BCryptPasswordEncoder());
//        Authentication authenticate = factory.createAuthenticationManager().authenticate(UsernamePasswordAuthenticationToken.unauthenticated(username, password));

        //校验用户名信息

        if (sysUserService.selectByUsername(username) == null) {
            log.info("save ldap user");
            // TODO 关联用户角色和权限
    ...
    private static OAuth2ClientAuthenticationToken getAuthenticatedClientElseThrowInvalidClient(Authentication authentication) {
        return getoAuth2ClientAuthenticationToken(authentication);
    }

    public static OAuth2ClientAuthenticationToken getoAuth2ClientAuthenticationToken(Authentication authentication) {
